...
// 模糊程度 参数范围 1~150
const blursize = 100
// 1：图片加蒙板 2：unsplash壁纸  3：Bing 壁纸 4 原生壁纸
const Imgstyle = 4
...
// 英文部分 字体、大小
const contentfont = new Font("Marker Felt", 16);
// 中文部分 字体、大小  
const notefont = new Font("Chalkduster", 12)
//##############公共参数配置模块-结束##############
...
const SentenceData = await getSentencedata()
...
if (!colorMode && !ImageMode && !config.runsInWidget && changePicBg) {
  ...
  if (response == 2) {
    Safari.openInApp(versionData['ONE-Motto'].wxurl, false);
  }
  if (response == 1) {
...
if (colorMode) {
  ...
  switch (Imgstyle) {
    ...
    case 3:
      ...
      break;
    case 4:
      const icibaurl = SentenceData.picture2
      const icibaImgs = await getImageByUrl(icibaurl, `_${Script.name()}-icibaImgs-bg`, false)
      bgImg = await shadowImage(icibaImgs)
      widget.backgroundImage = bgImg
      break;
  }
...
async function createWidget() {
  ...
  const row_ss = titleStack.addStack()
  row_ss.cornerRadius = 4
  row_ss.setPadding(2, 5, 2, 5)
  row_ss.backgroundColor = new Color("#000", 0.2)
  const wtitle = row_ss.addText(`每日一句`)
  ...
  // header.setPadding(2, 5, 2, 5)
  // header.backgroundColor = new Color("#000", 0.2)
  // header.centerAlignContent()     //https://i.loli.net/2021/06/15/zvpC3h1R4gSdt8b.png    https://game.gtimg.cn/images/lol/v3/logo-public.png
  // let logo = await getImageByUrl(`https://pic.rmb.bdstatic.com/bjh/24963e5f391d19e2c90c612fa9edc4cf.png`, `ONE-logo-bg`, false)
  // let logoElement = header.addImage(logo)
  // logoElement.imageSize = new Size(40, 40)
  // logoElement.cornerRadius = 4
  // logoElement.leftAlignImage()
  // header.addSpacer(4)
  // let title = header.addText(`${SentenceData.caption}`)
  ...
  const row = titleStack.addStack()
  const appIcon = SFSymbol.named("mic")
  const appIconElement = row.addImage(appIcon.image)
  appIconElement.imageSize = new Size(12, 12)
  appIconElement.tintColor = fontcolor
  row.addSpacer(2)
  const dateLine = row.addText(`跟读 >`);
  dateLine.font = Font.mediumSystemFont(12)
  dateLine.textColor = fontcolor
  dateLine.rightAlignText();
  dateLine.textOpacity = 0.7;

  dateLine.url = SentenceData.tts
  w.addSpacer(10)

  const footerStack = w.addStack()
  const linkStack = footerStack.addStack()
  linkStack.centerAlignContent()
  linkStack.layoutVertically()
  const content = linkStack.addText(` ${SentenceData.content}`);
  ...
  linkStack.addSpacer()
  w.addSpacer()
  const note = linkStack.addText(` ${SentenceData.note}`);
  note.font = notefont
  note.lineLimit = 3
  note.textColor = fontcolor
  note.centerAlignText()
  note.textOpacity = 0.7;



  w.url = SentenceData.tts
  w.addSpacer()
  return w
}
...
async function getSentencedata() {
  const SentenceCachePath = files.joinPath(files.documentsDirectory(), "Sentence-NK")
  var SentenceData
  try {
    let lolRequest = new Request(`http://open.iciba.com/dsapi/`);
    ...
    SentenceData = await lolRequest.loadJSON();
    files.writeString(SentenceCachePath, JSON.stringify(SentenceData))
    log("[+]每日一句数据获取成功")
  } catch (e) {
    SentenceData = JSON.parse(files.readString(SentenceCachePath))
    log("[+]请求失败，使用缓存数据")
  }
  return SentenceData
}
...
async function updateCheck(localversion) {
  ...
  let uC = versionData
  let originversion = uC['ONE-Motto'].version
  ...
  if (status == 1) {
    ...
    if (!config.runsInWidget) {
      ...
      upd.addAction("立即更新")
      upd.message = uC['ONE-Motto'].notes
      if (await upd.present() == 1) {
        const req = new Request(uC['ONE-Motto'].cdn_scriptURL)
