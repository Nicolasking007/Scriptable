...
// 模糊程度 参数范围 1~150
const blursize = 100
// 1： 表示图片加蒙板 2：表示 直接使用歌曲封面原图  3：用Bing 壁纸
const Imgstyle = 2
// 切换网易云热评接口数据源 
const Switch_data = false
// 上下左右间距 
const padding = {
  top: 10,
  left: 10,
...
if (!colorMode && !ImageMode && !config.runsInWidget && changePicBg) {
  ...
  if (response == 2) {
    Safari.openInApp(versionData['ONE-hotcomments'].wxurl, false);
  }
  if (response == 1) {
...
if (colorMode) {
  ...
  switch (Imgstyle) {
    case 1:
      const blugImgs = await getImageByUrl(Switch_data ? hotcommentsData.data.picurl : hotcommentsData.data.songPic, `_${Script.name()}-blugImgs-bg`, false)
      bgImg = await blurImage(blugImgs, blurStyle, blursize)
      widget.backgroundImage = bgImg
      break;
    case 2:
      const orginImgs = await getImageByUrl(Switch_data ? hotcommentsData.data.picurl : hotcommentsData.data.songPic, `_${Script.name()}-orginImgs-bg`, false)
      bgImg = await shadowImage(orginImgs)
      widget.backgroundImage = bgImg
      break;
    case 3:
      const url = "https://api.dujin.org/bing/1366.php"
      const bingImgs = await getImageByUrl(url, `_${Script.name()}-bingImgs-bg`, false)
...
async function createWidget() {
  ...
  let docsSymbol = SFSymbol.named("tortoise.fill") //tortoise.fill
...
async function updateCheck(localversion) {
  ...
  let uC = versionData
  let originversion = uC['ONE-hotcomments'].version
