...
// 关注的球队id 湖人13 在此修改或桌面组件Parameter处 详见公众号配置
const teamId = args.widgetParameter || '17' 
// 显示内容数量   
const maxEvents = previewSize === "large" ? 8 : 4  
// 上下左右间距
const padding = {
  top: 0,
  left: 0,
...
const _datenvet = new Date().format("yyyy-MM")
...
if (!colorMode && !ImageMode && !config.runsInWidget && changePicBg) {
    ...
    if (response == 2) {
        Safari.openInApp(versionData['ONE-NBAMATCH'].wxurl, false);
    }
    if (response == 1) {
...
async function createWidget() {
  ...
  for (var i=0;i<keys.length;i++) {
    select_key = keys[i];//数组的索引是从0开始的

  }

  if (previewSize == 'large') {

    const teamName = events[select_key][events[select_key].length - 1].matchDesc
    w.addSpacer(10)
    ...
    _title.font = new Font("Chalkduster", 16) //Font.boldRoundedSystemFont(16);
    header.addSpacer()
  ...
  if (select_key == _datenvet) {

    const limitedEvents = events[_datenvet].slice(0, maxEvents)
    for (let i = 0; i < limitedEvents.length; i++) {
      let e = events[select_key].slice(-maxEvents)
      let competitors = e[i];
      ...
      if (Number(competitors.rightGoal) > Number(competitors.leftGoal)) {
        ...
        homescore.font = new Font("Chalkduster", 12)
      ...
      if (Number(competitors.leftGoal) > Number(competitors.rightGoal)) {
        ...
        awayscore.font = new Font("Chalkduster", 12)
