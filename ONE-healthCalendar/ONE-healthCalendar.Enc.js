...
async function createWidget() {
  ...
  if (previewSize === "small") {
    ...
    let resultBarImage = getResultBar(resultColor);
    let imageElement = topStack.addImage(resultBarImage);
    imageElement.imageSize = new Size(5, 50); // 调整结果条的大小
    topStack.addSpacer(10)

    let titleStack = topStack.addStack();
    titleStack.layoutVertically();

    let title1Stack = titleStack.addStack();
    title1Stack.layoutHorizontally();
    title1Stack.topAlignContent();

    // 创建第一个标题的第一部分
    let title1_1 = title1Stack.addText(`${CVTData.data.items[0].health_festival_desc} `);
    title1_1.font = Font.lightSystemFont(12);
    title1_1.textColor = fontcolor; // 设置第一个部分的字体颜色

    // 添加一些空间来分隔两个部分
    title1Stack.addSpacer(5);

    // 创建第一个标题的第二部分
    let title1_2 = title1Stack.addText(`${CVTData.data.items[0].show_date_simple_str}期`);
    title1_2.font = Font.lightSystemFont(12);
    title1_2.textColor = Color.orange(); // 设置第二个部分的字体颜色

    // // 添加第一个标题到垂直堆栈
    // let title1 = titleStack.addText(`  .  `);
    // title1.font = Font.lightSystemFont(12)
    // title1.textColor = fontcolor;

    // 添加一些空间来分隔两个标题
    titleStack.addSpacer(5);

    // 添加第二个标题到垂直堆栈
    let title2 = titleStack.addText(CVTData.data.items[0].health_view);
    title2.font = Font.boldSystemFont(10);
    title2.textColor = fontcolor;

    topStack.addSpacer(); // 填充剩余空间以将标题和结果条居左
    // 添加文本区域
    widget.addSpacer(5); // 在顶部堆栈和文本区域之间添加较小的空间

    const deviceLine = widget.addText(CVTData.data.items[0].analy_context)
    deviceLine.font = Font.mediumSystemFont(8)
    deviceLine.lineLimit = 5
    deviceLine.textColor = fontcolor
    ...
    // 创建第一个标题的第一部分
    let title1_1 = title1Stack.addText(`${CVTData.data.items[0].health_festival_desc} •`);
    title1_1.font = Font.lightSystemFont(16);
    title1_1.textColor = fontcolor; // 设置第一个部分的字体颜色
    ...
    // 创建第一个标题的第二部分
    let title1_2 = title1Stack.addText(`${CVTData.data.items[0].show_date_simple_str}期`);
    title1_2.font = Font.lightSystemFont(16);
    title1_2.textColor = Color.orange(); // 设置第二个部分的字体颜色
    ...
    // 添加第二个标题到垂直堆栈
    let title2 = titleStack.addText(CVTData.data.items[0].health_view);
    title2.font = Font.boldSystemFont(14);
    title2.textColor = fontcolor;

    topStack.addSpacer(); // 填充剩余空间以将标题和结果条居左
    // 添加文本区域
    widget.addSpacer(5); // 在顶部堆栈和文本区域之间添加较小的空间
    const dateLine = topStack.addText(`↻ ${timeFormatter.string(new Date())}`);
...
async function getVcT_TopData() {
  const VTDataCachePath = files.joinPath(files.documentsDirectory(), "healthCalendarData-NK")
  var VTData
  try {
    VTData = await new Request(atob("aHR0cHM6Ly9keHkuY29tL3ZpZXcvaS9jb250L2hlYWx0aC9oZWFsdGhDYWxlbmRhci9pbmZv")).loadJSON()
...
function getResultBar(resultColor) {
  ...
  drawContext.size = new Size(5, 50); // 调整结果条的大小
  drawContext.respectScreenScale = true;
  drawContext.opaque = false;
  drawContext.setStrokeColor(resultColor);
  drawContext.setLineWidth(5);
  const path = new Path();
  path.move(new Point(2.5, 2.5));
  path.addLine(new Point(2.5, 47.5));
  drawContext.addPath(path);
  drawContext.strokePath();
  drawContext.setFillColor(resultColor);
  drawContext.fillEllipse(new Rect(0, 0, 5, 5));
  drawContext.fillEllipse(new Rect(0, 45, 5, 5));
...
async function updateCheck(localversion) {
  ...
  let uC = versionData
  let originversion = uC['ONE-healthCalendar'].version
  ...
  if (status == 1) {
    ...
    if (!config.runsInWidget) {
      ...
      upd.addAction("立即更新")
      upd.message = uC['ONE-healthCalendar'].notes
      if (await upd.present() == 1) {
        const req = new Request(uC['ONE-healthCalendar'].cdn_scriptURL)
